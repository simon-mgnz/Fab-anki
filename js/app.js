// Single-file app implementing requested functions.
  ...
  function fixMojibakeText(value){
    try{
      ...
      const toCp1252Bytes = (input) => {
        const out = new Uint8Array(input.length);
        for(let i = 0; i < input.length; i++){
          const code = input.charCodeAt(i);
          if(code <= 0xFF){
            out[i] = code;
          }else if(Object.prototype.hasOwnProperty.call(cp1252Map, code)){
            out[i] = cp1252Map[code];
          }else{
            // Keep a harmless placeholder byte for unknown symbols.
            out[i] = 0x3F;
          }
        }
        return out;
      ...
      // Some strings are double-encoded; decode up to a few passes.
      for(let i = 0; i < 3; i++){
        if(!suspect.test(str)) break;
        const bytes = toCp1252Bytes(str);
        const decoded = new TextDecoder('utf-8', { fatal: false }).decode(bytes);
        if(!decoded || decoded === str) break;
        str = decoded;
      }
      return str;
    }catch(e){
  ...
  function normalizeDomMojibake(rootNode){
    try{
      const root = rootNode || document.body;
      if(!root) return;

      const normalizeElementAttrs = (el) => {
        if(!el || el.nodeType !== 1) return;
        const attrs = ['title', 'placeholder', 'aria-label', 'data-label-text', 'data-label-emoji'];
      ...
      if(root.nodeType === 3){
        const fixed = fixMojibakeText(root.nodeValue);
        if(fixed !== root.nodeValue) root.nodeValue = fixed;
        return;
      }

      normalizeElementAttrs(root);
  ...
  function installMojibakeDomObserver(){
    try{
      if(window.__fabankiMojibakeObserverInstalled) return;
      window.__fabankiMojibakeObserverInstalled = true;

      const run = () => normalizeDomMojibake(document.body);
      ...
      const observer = new MutationObserver((mutations) => {
        for(const mutation of mutations){
          if(mutation.type === 'characterData'){
            normalizeDomMojibake(mutation.target);
            continue;
          }
      ...
      observer.observe(document.documentElement || document.body, {
        ...
        characterData: true,
        attributes: true,
        attributeFilter: ['title', 'placeholder', 'aria-label', 'data-label-text', 'data-label-emoji']
      });
      window.__fabankiMojibakeObserver = observer;
    }catch(e){}
  }

  function normalizeI18nStrings(){
    try{
      for(const lang of Object.keys(_i18n || {})){
        const dict = _i18n[lang] || {};
        for(const key of Object.keys(dict)){
          if(typeof dict[key] === 'string') dict[key] = fixMojibakeText(dict[key]);
        }
      }
    }catch(e){}
  }
  normalizeI18nStrings();
  installMojibakeDomObserver();

  function t(key) {
    const lang = localStorage.getItem('fabanki:lang') || 'fr';
    const raw = (_i18n[lang] && _i18n[lang][key]) || (_i18n.fr && _i18n.fr[key]) || key;
    return fixMojibakeText(raw);
  }
  ...
  function showDeckLoading(active, message){
    try{
      const existing = document.getElementById('deckLoadingOverlay');
      if(!active){
        if(existing) existing.remove();
        return;
      }
      if(existing){
        const msg = existing.querySelector('.deck-loading__message');
        if(msg) msg.textContent = message || t('downloadingDeck');
        return;
      }

      const overlay = document.createElement('div');
      overlay.id = 'deckLoadingOverlay';
      overlay.className = 'deck-loading-overlay';

      const card = document.createElement('div');
      card.className = 'deck-loading-card';

      const spinner = document.createElement('div');
      spinner.className = 'deck-loading-spinner';

      const title = document.createElement('div');
      title.className = 'deck-loading__title';
      title.textContent = 'Chargement du deck';

      const msg = document.createElement('div');
      msg.className = 'deck-loading__message';
      msg.textContent = message || t('downloadingDeck');
  ...
  function updateUILanguage(langCode) {
    try {
      ...
      elementsToTranslate.forEach(({ id, key }) => {
        ...
        if (el && tr[key]) {
          const safeLabel = fixMojibakeText(tr[key]);
          el.textContent = safeLabel;
          if (el.setAttribute) {
            el.setAttribute('data-label-text', safeLabel);
          }
        }
      });
      
      // Update profile popup if it's open
      if(typeof updateProfilePopupIfOpen === 'function') {
  ...
  function getCredits(){ 
    try {
      return Number(localStorage.getItem('fabanki:credits') || 0);
    } catch(e) {
      console.error('[CREDITS] Failed to get credits:', e);
      return 0;
    }
  }
  ...
  function addCredits(amount){
    try{
      ...
      console.log('[CREDITS] Added', amount, 'credits. New total:', total);
      // Auto-sync after credits change
      setTimeout(() => {
        if(typeof autoSync === 'function'){
          autoSync().catch(e => console.warn('autoSync failed:', e));
        }
      }, 100);
      return amount;
    }catch(e){ 
  ...
  function getXpTotal(){ 
    try {
      return Number(localStorage.getItem('fabanki:xp_total') || 0);
    } catch(e) {
      console.error('[XP] Failed to get XP:', e);
      return 0;
    }
  }
  ...
  function computeLevelAndProgress(totalXp){
    ...
    try {
      ...
      console.error('[XP] Failed to compute level:', e);
      return { level: 1, progress: 0, need: 100, toNext: 100, pct: 0 };
    }
  }
  ...
  function showModeSelectionTooltip(modes, deckUrl, tempKey, triggerButton = null){
    const existing = document.getElementById('modeSelectionTooltip');
    if(existing) existing.remove();
  ...
  async function openDeckWithModeSelection(deckUrl, deckTitle, triggerButton = null){
    ...
    try{
      ...
      const reverseUnlocked = (typeof isReverseModeUnlocked === 'function')
        ? isReverseModeUnlocked()
        : (localStorage.getItem('fabanki:mode_reverse_unlocked') === 'true');
      ...
      const fallbackModes = [
        {id: 'default', name: 'Anki', unlocked: true}
        // Removed activeMemory from fallback
      ];
      showModeSelectionTooltip(fallbackModes, deckUrl, tempKey, triggerButton);
    }
  }
      ...
      if(!res.ok){
        if(res.status === 404){
          showNotFoundScreen({
            ...
            message: 'Ce deck est introuvable. Verifiez le chemin ou rechargez la liste.',
    ...
    try{
      ...
      if((deckHasTextTag || deckHasTimerTag) && !lockState.locked){
        ...
        const reverseUnlocked = (typeof isReverseModeUnlocked === 'function')
          ? isReverseModeUnlocked()
          : (localStorage.getItem('fabanki:mode_reverse_unlocked') === 'true');
        const reverseLockedByQuest = !reverseUnlocked;
  ...
  function clearErrorScreen(){
    try{
      const overlay = document.getElementById('errorScreenOverlay');
    ...
    try{
      ...
      if(options.className){
        const classes = Array.isArray(options.className)
          ? options.className
          : String(options.className).split(' ').filter(Boolean);
        classes.forEach(c => wrap.classList.add(c));
      }
      wrap.classList.add('error-screen--overlay');
      ...
      if(typeof options.onAction === 'function'){
        ...
        if(options.actionClass) retry.className = options.actionClass;
      ...
      if(options.showBrowseButton !== false){
        const browse = document.createElement('button');
        browse.className = 'secondary';
        browse.textContent = t('browseDecksBtn');
      ...
      if(options.showHomeButton !== false){
        const home = document.createElement('button');
        home.className = 'secondary';
        home.textContent = t('homeReturn');
      ...
      const overlay = document.createElement('div');
      overlay.id = 'errorScreenOverlay';
      overlay.className = 'error-screen-overlay';
      overlay.appendChild(wrap);
      document.body.appendChild(overlay);
    }catch(e){ console.warn('showErrorScreen error:', e); }
  }

  // Global error capture to ensure the error screen is visible on uncaught failures.
  (function setupGlobalErrorScreen(){
    if(window.__fabanki_global_error_screen) return;
    window.__fabanki_global_error_screen = true;
    let lastErrorAt = 0;
    const showGlobal = (title, message, details) => {
      const now = Date.now();
      ...
      try{
        showErrorScreen({
          ...
          message: message || 'Une erreur est survenue pendant l\'execution.',
          details: details || ''
        });
      }catch(e){}
    };
    ...
    window.addEventListener('error', (event) => {
      try{
        const msg = event && event.message ? event.message : 'Erreur JavaScript';
        const detail = event && event.filename ? `${event.filename}:${event.lineno || 0}:${event.colno || 0}` : '';
        showGlobal('Erreur JavaScript', msg, detail);
      }catch(e){}
    });
  ...
  function showNotFoundScreen(options = {}){
    const details = options.details || 'Le fichier demande est introuvable (404).';
    showErrorScreen({
      icon: '404',
      iconClass: 'error-screen__icon--code',
      className: ['error-screen--page', 'error-screen--404'],
      title: options.title || 'Page introuvable',
      message: options.message || 'Le contenu demande n\'existe pas ou a ete deplace.',
      details,
      actionLabel: options.actionLabel || 'Retour accueil',
      actionClass: 'primary',
      onAction: options.onAction || (()=>{ try{ window.location.href = 'https://fabanki.fr/'; }catch(e){ if(typeof renderWelcomeDecks === 'function') renderWelcomeDecks(); } }),
  ...
  function renderEmpty(options = {}){
    ...
    if(!sessionCreditsGranted && totalReviewed > 0){
      ...
      if(grant > 0 && typeof addCredits === 'function'){
        ...
        sessionCreditsGrantedAmount = added; // Save actual amount granted
    ...
    // Use the actual credits granted amount from earlier in the session
    // (already calculated and saved when credits were granted)
    ...
    if(front){
      ...
      if(!skipSessionTracking && reviewedCount >= 1){
        ...
        try{
          ...
          if(state){
            // Increment review sessions (Part 4 quest: "Complete 25 sessions")
            state.part4_reviewSessions = (Number(state.part4_reviewSessions || 0)) + 1;
            
            // NOTE: Card-based tracking (part4_totalCards, part4_timerCards, part4_maintenanceCards,
            // part5_multipleCards, part5_calculCards, part5_totalCards) is handled per-card in
            // syncPostWelcomeQuestProgress() during review to accurately track per-mode counts.
            // Session-end tracking here would be inaccurate if modes were switched during session.
            
            // Check reward for sessions quest only (other rewards checked in syncPostWelcomeQuestProgress)
  ...
  function showNextCard(){
    ...
    try{
      ...
      if(homeBtn){
        homeBtn.style.display = 'inline-block';
        if(syncBtn && syncBtn.parentNode && homeBtn.parentNode !== syncBtn.parentNode){
          const parent = syncBtn.parentNode;
          const syncBtnNextSibling = syncBtn.nextSibling;
          parent.insertBefore(homeBtn, syncBtnNextSibling);
        }
  ...
  function displayClassicQuestion(q, card){
    ...
    showAnswerBtn.textContent = 'Voir la rÃ©ponse';
    ...
    const gradeButtons = [
      {label: 'RatÃ©', quality: 0, color: '#ef4444'},
        ...
        if(reviewMode === 'rush' && window.rushModeState){
          ...
          if(respButtons){
            ...
            if(window.rushModeState.timerExpired){
              ...
              // Timer didn't expire - show all buttons
              buttons.forEach(btn => {
                btn.style.display = '';
              });
            }
  ...
  window.showModificationRequests = function(){
    try{
      ...
      const container = document.createElement('div');
      container.style.cssText = 'max-width:1200px;margin:0 auto;background:var(--card-bg);padding:24px;border-radius:12px;position:relative;';
  ...
  function passCurrent(){
    try{
      ...
      if(typeof autoSync === 'function') autoSync().catch(e => syncLog('autoSync in passCurrent failed:', e));
  ...
  function answerCurrent(q, options = {}){
      ...
      if(q >= 3){  // only count correct answers
        ...
        try{
          if(typeof updateWelcomeQuest === 'function'){
            ...
            const part1Done = questState.part1.onboarding_done && questState.part1.profile_opened && questState.part1.market_opened && questState.part1.goal_set;
            if(part1Done){
    ...
    try{
      ...
      try{
        ...
        const maxNoPass = Math.max(Number(localStorage.getItem('fabanki:consec_no_pass_max')||0), consecNoPass);
      ...
      if([3,4,5].includes(q)){
        try{
          const timeSec = Math.max(0, (Date.now() - (cardShownAt||Date.now())) / 1000);
                    ...
                    // Time already accumulated above
    ...
    // Sync card review progress to cloud
    if(typeof autoSync === 'function') autoSync().catch(e => syncLog('autoSync in answerCurrent failed:', e));
  ...
  try{ setReviewTopBarVisibility(!!isReviewing); }catch(e){}
    ...
    function showProfilePopup(){
      try{
        ...
        try{
          ...
          for(const lang of langs){
            ...
            btn.addEventListener('click', () => {
              ...
              try{ if(typeof showWelcomeQuestToast === 'function') showWelcomeQuestToast(clean(`Langue changee en ${lang.label}`)); }catch(e){}
            });
        ...
        if(unlockedTitles.length > 0){
          ...
          noneOption.value = '';
          noneOption.textContent = t('noTitle');
    ...
    function updateProfilePopupIfOpen(){
      try{
        ...
        try{
          ...
          if(nextTxt) nextTxt.textContent = `${t('nextLevelIn')} ${lvl.toNext} ${t('xpLabel')}`;
    ...
    async function mergeAllClassementDuplicates(dryRun = true, silent = false){
      try{
        const db = window.__fabanki_firestore;
        if(!db) return { mergedUsers: 0, removedDocs: 0, skipped: true };
        ...
        for(const entry of entries){
          const pseudoRaw = String(entry.data.Pseudo || '').trim();
          if(!pseudoRaw) continue;
          const key = pseudoRaw.toLowerCase();
          if(!groups[key]) groups[key] = [];
          groups[key].push(entry);
        }

        const duplicates = Object.values(groups).filter(arr => arr.length > 1);
        if(!silent) console.log(`[MERGE-ALL] duplicate groups: ${duplicates.length}`);
        if(duplicates.length === 0) return { mergedUsers: 0, removedDocs: 0 };
        if(dryRun) return { mergedUsers: duplicates.length, removedDocs: 0, dryRun: true };

        const numericFields = [
          'XP', 'Niveau', 'Cartes r?vis?es', 'Bonnes r?ponses', 'Rat?s', 'Passer',
          'Streak_max', 'Streak_current', 'Daily_goal', 'Cartes ma?tris?es',
          'Qu?tes_quotidiennes', 'Score_MPSI', 'XP_semaine', 'Score_MPSI_semaine', 'Score_MPSI_mois'
        ];
        ...
        for(const docs of duplicates){
          let target = docs[0];
          for(const d of docs){
            const aCards = Number(target.data['Cartes r?vis?es'] || 0);
            const bCards = Number(d.data['Cartes r?vis?es'] || 0);
            const aScore = Number(target.data.Score_MPSI || 0);
            const bScore = Number(d.data.Score_MPSI || 0);
            if(bCards > aCards || (bCards === aCards && bScore > aScore)) target = d;
          }

          const mergedDoc = { ...(target.data || {}) };
          for(const f of numericFields){
            mergedDoc[f] = docs.reduce((sum, d) => sum + Number(d.data?.[f] || 0), 0);
          }

          const mergeTextArray = (field) => {
            const set = new Set();
            for(const d of docs){
              const arr = Array.isArray(d.data?.[field]) ? d.data[field] : [];
              for(const v of arr){
          ...
          mergedDoc.Badges = mergeTextArray('Badges');
          mergedDoc.Objectifs = mergeTextArray('Objectifs');
          mergedDoc.Titres = mergeTextArray('Titres');
          mergedDoc['Derni?re synchronisation'] = new Date().toISOString();
          mergedDoc['Derni?re mise ? jour'] = new Date().toISOString();

          await db.collection('Classement').doc(target.id).set(sanitizeAndValidateUserData(mergedDoc), { merge: true });

          for(const d of docs){
            if(d.id !== target.id){
              await db.collection('Classement').doc(d.id).delete();
              removedDocs++;
            }
          }
        ...
        if(!silent) console.log(`[MERGE-ALL] done. users=${mergedUsers}, removed=${removedDocs}`);
        return { mergedUsers, removedDocs };
      }catch(e){
        const errCode = String(e?.code || '');
        const errMsg = String(e?.message || e || '');
        const isPermissionError = errCode === 'permission-denied' || /insufficient permissions|permission denied/i.test(errMsg);
        if(!silent || !isPermissionError){
    ...
    function applySimonBonusIfNeeded(){
      try{
    ...
    async function showMarketPage(){
      try{
        ...
        const carouselNavBtns = document.createElement('div');
        ...
        carouselTrack.appendChild(textModeSection);
        ...
        carouselTrack.appendChild(timerModeSection);
        ...
        carouselTrack.appendChild(rushModeSection);
        ...
        const cardPatternsMarket = [
          { name: 'Topographie', id: 'hero_topography', pattern: "url(\"data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='120' height='120' viewBox='0 0 120 120'><path d='M0 60 Q30 40 60 60 T120 60' fill='none' stroke='%2399a' stroke-opacity='0.30' stroke-width='2'/><path d='M0 80 Q30 60 60 80 T120 80' fill='none' stroke='%2399a' stroke-opacity='0.20' stroke-width='2'/><path d='M0 40 Q30 20 60 40 T120 40' fill='none' stroke='%2399a' stroke-opacity='0.20' stroke-width='2'/></svg>\")", level: 35, credit: 35 },
          { name: 'Hexagones', id: 'hero_hex', pattern: "url(\"data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='100' height='90' viewBox='0 0 100 90'><path d='M25 5 L75 5 L95 45 L75 85 L25 85 L5 45 Z' fill='none' stroke='%2399a' stroke-opacity='0.30' stroke-width='2'/></svg>\")", level: 40, credit: 40 },
          { name: 'Circuit', id: 'hero_circuit', pattern: "url(\"data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='120' height='120' viewBox='0 0 120 120'><path d='M10 20 H60 V10 H110' fill='none' stroke='%2399a' stroke-opacity='0.30' stroke-width='2'/><path d='M10 80 H50 V110 H110' fill='none' stroke='%2399a' stroke-opacity='0.20' stroke-width='2'/><circle cx='60' cy='10' r='3' fill='%2399a' fill-opacity='0.35'/><circle cx='50' cy='110' r='3' fill='%2399a' fill-opacity='0.35'/><circle cx='60' cy='20' r='3' fill='%2399a' fill-opacity='0.30'/></svg>\")", level: 50, credit: 50 },
          { name: 'Puzzle', id: 'hero_jigsaw', pattern: "url(\"data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='120' height='120' viewBox='0 0 120 120'><path d='M10 10 H50 V30 H70 V10 H110 V110 H70 V90 H50 V110 H10 Z' fill='none' stroke='%2399a' stroke-opacity='0.30' stroke-width='2'/><circle cx='60' cy='30' r='6' fill='none' stroke='%2399a' stroke-opacity='0.30' stroke-width='2'/></svg>\")", level: 60, credit: 60 }
        ];
        ...
        */
        ...
        for(const [tierKey, tierData] of Object.entries(mathematicianTiers)){
          // Check which mathematicians in this tier are unpurchased
          const unpurchasedMath = tierData.mathematicians.filter(mathName => {
            const id = `title_${mathName.toLowerCase()}`;
            return !localStorage.getItem(`fabanki:market_${id}`);
          });
          
          // Skip tier if all mathematicians are purchased
          if(unpurchasedMath.length === 0) {
            console.log(`DEBUG Tier ${tierData.name}: All mathematicians purchased, hiding tier`);
            continue;
          }
          
          // Display one random unpurchased mathematician from this tier
          const randIndex = Math.floor(Math.random() * unpurchasedMath.length);
          const displayMath = unpurchasedMath[randIndex];
          const itemId = `title_${displayMath.toLowerCase()}`;
          
          console.log(`DEBUG Title ${displayMath} (Tier ${tierData.name}): Showing (${unpurchasedMath.length}/${tierData.mathematicians.length} available)`);
    ...
    function initWelcomeQuest(){
      ...
      if(existing && !existing.rewarded){
        existing.rewarded = { part1: {}, part2: {}, part3: {} };
        localStorage.setItem('fabanki:welcome_quest', JSON.stringify(existing));
      }
      if(existing){
    ...
    const welcomeQuestRewards = {
      part1: { onboarding_done: 5, profile_opened: 3, market_opened: 3, goal_set: 3 },
      part2: { deck_opened: 3, cards_50_reviewed: 5, session_completed: 5 },
      part3: { daily_goal_completed: 10, market_purchase: 10, first_title: 10, three_daily_quests: 10, active_memory_launched: 10 }
    };

    function isWelcomeQuestPartUnlocked(state, part){
      if(part === 'part1') return true;
      ...
      if(part1Done){
        grantWelcomeQuestPendingRewards(state, 'part2');
      }
      if(part2Done){
        grantWelcomeQuestPendingRewards(state, 'part3');
      }
      if(part === 'part2') return part1Done;
      if(part === 'part3') return part2Done;
      return true;
    }

    function isWelcomeQuestRewarded(state, part, mission){
      return !!(state.rewarded && state.rewarded[part] && state.rewarded[part][mission]);
    }

    function markWelcomeQuestRewarded(state, part, mission){
      if(!state.rewarded) state.rewarded = { part1: {}, part2: {}, part3: {} };
      if(!state.rewarded[part]) state.rewarded[part] = {};
      state.rewarded[part][mission] = true;
    }

    function grantWelcomeQuestPendingRewards(state, part){
      if(!state || !state[part]) return;
      if(!isWelcomeQuestPartUnlocked(state, part)) return;
      const rewards = welcomeQuestRewards[part] || {};
      Object.keys(rewards).forEach(mission => {
        if(state[part][mission] && !isWelcomeQuestRewarded(state, part, mission)){
    ...
    function completeWelcomeQuestMission(part, mission, creditReward){
      ...
      const unlocked = isWelcomeQuestPartUnlocked(state, part);
      const rewardMap = welcomeQuestRewards[part] || {};
    ...
    function createRewardCardUI({ title, subtitle, locked, previewStyle, details, previewEmoji }){
      const card = document.createElement('div');
    ...
    function createQuestPart(title, missions, showProgress, locked = false){
      ...
      missions.forEach(mission => {
        ...
        label.style.cssText = `color:${displayDone ? '#6b7280' : '#1f2937'};text-decoration:${displayDone ? 'line-through' : 'none'};font-size:0.9em;`;
        label.textContent = mission.label;
        leftSide.appendChild(label);
        
        if(mission.progress && !displayDone && !locked){
    ...
    function syncPostWelcomeQuestProgress({ mode, timeSec, reviewed, deckKey, masteredDelta, quality }){
      ...
      try{
        ...
        const part2Unlocked = part1Done;
        let changed = false;
        if(reviewed){
          if(part2Unlocked && state.totalCards < postWelcomeQuestTargets.totalCards){
            state.totalCards += 1;
            changed = true;
          }
          ...
          // Part 4 tracking: Total cards (when Part 3 is complete)
          try{
            const decksReviewedCount = (state.part3_decksReviewedSet || '').split(',').filter(x => x).length;
            const part3Done = state.part3_timeSec >= postWelcomeQuestTargets.part3_timeSec &&
              decksReviewedCount >= postWelcomeQuestTargets.part3_decksReviewed &&
            ...
            if(part3Done && state.part4_totalCards < postWelcomeQuestTargets.part4_totalCards){
              state.part4_totalCards = (Number(state.part4_totalCards || 0)) + 1;
              changed = true;
            }
          }catch(e){
            console.error('[Quest Error] Part 4 tracking failed:', e);
          }
          ...
          // Track all reviewed cards for the "2000 cartes au total" quest
          if(['default', 'fillblank', 'rush', 'associer', 'step', 'activeMemory', 'reverse', 'random', 'timer', 'hold', 'multiple', 'calcul', 'original'].includes(mode)){
        ...
        if(part2Unlocked && state.timerUnlocked && !state.timerRewarded){
          addCredits(10);
    ...
    try{ 
      window.renderPostWelcomeQuestCard = renderPostWelcomeQuestCard;
      window.syncPostWelcomeQuestProgress = syncPostWelcomeQuestProgress;
    }catch(e){}
    ...
    // Backwards-compatible alias used by sync code
    function collectDeckProgress(){
      try{ return collectDeckStates(); }catch(e){ return {}; }
    }
    ...
    function collectInventory(){
      try{
        const inventory = {};
        for(const k of Object.keys(localStorage)){
          // Collect persistent unlocks/settings that must survive device changes/reinstall
          if(
    ...
    function mergeDeckStates(localDecks, remoteDecks){
      ...
      for(const dKey of Object.keys(localDecks || {})){
        ...
        // Merge cards: take the card state with the most recent 'last' timestamp
        for(const cId of Object.keys(localDecks[dKey]?.cards || {})){
          ...
          if(!remoteCard){
            ...
            merged[dKey].cards[cId] = localCard;
          } else {
            // Card in both: take the one with more recent 'last' review
            const localLast = new Date(localCard.last || 0).getTime();
            const remoteLast = new Date(remoteCard.last || 0).getTime();
    ...
    function syncDebugEnabled(){
      // Disable sync debug logging by default - too verbose
      // Set localStorage.setItem('fabanki:sync_debug', '1') to enable
      try{ return (localStorage.getItem('fabanki:sync_debug') === '1'); }catch(e){ return false }
    }
    function syncLog(...args){ if(syncDebugEnabled()) console.log('[SYNC]', ...args); }
    function ensureSyncStatusEl(){
      try{
        ...
        el.style.cssText = 'position:fixed;right:12px;bottom:12px;z-index:3000;background:rgba(15,23,42,0.92);color:#fff;padding:8px 12px;border-radius:8px;font-size:0.85rem;box-shadow:0 6px 16px rgba(0,0,0,0.25);display:none;';
    ...
    function showSyncLoading(active, message){
      try{
        const existing = document.getElementById('syncLoadingOverlay');
        if(!active){
          if(existing) existing.remove();
          return;
        }
        ...
        overlay.className = 'sync-loading-overlay';

        const card = document.createElement('div');
        card.className = 'sync-loading-card';

        const spinner = document.createElement('div');
        spinner.className = 'sync-loading-spinner';

        const title = document.createElement('div');
        title.className = 'sync-loading__title';
    ...
    window.__fabanki_testSync = async function(){
      console.log('=== FABANKI SYNC TEST ===');
      console.log('Mode:', localStorage.getItem('fabanki:mode'));
      console.log('User ID:', localStorage.getItem('fabanki:user_id'));
      ...
      if(typeof autoSync === 'function'){
        ...
        console.log('autoSync not available');
      }
    };
    window.__fabanki_enableDebug = function(){
      localStorage.setItem('fabanki:sync_debug', '1');
      window.__fabanki_sync_debug = true;
      console.log('Sync debug enabled - check console for [SYNC] messages');
    };
    window.__fabanki_disableDebug = function(){
      localStorage.removeItem('fabanki:sync_debug');
      window.__fabanki_sync_debug = false;
    ...
    async function autoSync(){
      try{
        ...
        if(pseudo === 'Anonyme' || !pseudo || pseudo.trim() === ''){
          syncLog('autoSync: skipping - anonymous user');
          return;
        }
        ...
        // Build current state from localStorage (now merged with any remote updates)
    ...
    // Keep startup merge non-destructive: leaderboard dedupe is now handled client-side.
    setTimeout(async () => {
      try{
        if(localStorage.getItem('fabanki:mode') === 'synced' && window.__fabanki_firestore){
          await mergeAllClassementDuplicates(true, true);
        }
      }catch(e){}
    }, 600);
        ...
        const updateMission = (missions, missionId, progress, type) => {
          ...
          if(reachedTarget && !mission.completed){
            ...
            if(type === 'daily' && !mission.welcome_counted){
              try{
                const questState = initWelcomeQuest();
                const part2Done = questState.part2.deck_opened && questState.part2.cards_50_reviewed && questState.part2.session_completed;
                if(!part2Done){
  ...
  `, level: 42 },
        ...
        cardPatternTitle.textContent = 'Motif des cartes';
    ...
    function applyCardPattern() {
      try {
        ...
        // Get current card color to preserve it
        const cardColor = localStorage.getItem('fabanki:card_color');
        const isDark = document.documentElement.getAttribute('data-theme') === 'dark';
        ...
        cards.forEach(card => {
          ...
          if (cardPattern !== 'none') {
            ...
            if (patternMap[cardPattern]) {
              ...
              // When pattern is active, ensure there's a background for the pattern to show on
              if (cardColor && !cardColor.includes('gradient')) {
                // Use custom color as background
                card.style.backgroundColor = cardColor;
                card.style.backgroundImage = ''; // Clear inline style to let CSS pattern show
              } else if (cardColor && cardColor.includes('gradient')) {
                // For gradients, patterns won't work well, so skip pattern and just use gradient
                let appliedGradient = cardColor;
                if (cardColor.includes(', light:') || cardColor.includes('), dark:')) {
                  const parts = cardColor.split('), ');
                  if (parts.length >= 2) {
                ...
                card.style.backgroundImage = appliedGradient;
                card.style.backgroundColor = 'transparent';
                // Remove pattern class since gradient is incompatible
                card.classList.remove(patternMap[cardPattern]);
              } else {
                // No custom color - keep card default background so pattern is visible
                // Don't clear backgroundColor, let CSS default handle it
                card.style.backgroundImage = ''; // Clear inline to let CSS pattern show
              }
            }
          } else {
            // No pattern - ensure inline background-image is cleared so card color can work
    ...
    function accumulateTimeSpent(timeSec){
      try{
        const addSec = Math.max(0, Math.round(timeSec || 0));
        if(addSec <= 0) return;
    ...
    function tierToRoman(t){ const map=['','I','II','III','IV','V','VI','VII','VIII','IX','X']; return map[Number(t)||0]||String(t); }
    ...
    function computeTitles(stats){
      ...
      try{
        ...
        const defs = [
          {nom:'Gauss', th:[100,300,600,1000,2000,3500,5500,8000,11000,15000], val: bonnes},
          {nom:'Fourier', th:[200,600,1200,2500,5000,10000,15000,22000,30000,40000], val: reviewed},
          {nom:'Euler', th:[200,500,1000,2000,4000,7000,10000,15000,20000,30000], val: (reviewed>0? Math.round((bonnes/reviewed)*100):0), isPercent:true, percentDefs:[0.75,0.80,0.85,0.90,0.92,0.93,0.94,0.95,0.96,0.97]},
    ...
    function showLeaderboardPopup(){
      try{
        ...
        function isValidLeaderboardEntry(d) {
          try {
            const level = Number(d.Niveau || d['Niveau PrÃ©pa'] || d['Niveau Prépa'] || 0);
        ...
        function getLeaderboardField(entry, aliases, fallback){
          ...
          const wanted = new Set((aliases || []).map(normalizeLeaderboardKey));
          for(const [k, v] of Object.entries(entry)){
            if(wanted.has(normalizeLeaderboardKey(k))) return v;
          }
          return fallback;
        }
        ...
        function renderRow(d, rank, scoreField = 'Score_MPSI'){
          ...
          const cardsReviewed = Number(getLeaderboardField(d, ['Cartes rÃ©visÃ©es', 'Cartes révisées', 'Cartes revisees'], 0) || 0);
          ...
          const tdNiv = document.createElement('td'); tdNiv.textContent = getLeaderboardField(d, ['Niveau', 'Niveau PrÃ©pa', 'Niveau Prépa', 'Niveau Prepa'], '');
          ...
          const tdCartes = document.createElement('td'); tdCartes.textContent = getLeaderboardField(d, ['Cartes rÃ©visÃ©es', 'Cartes révisées', 'Cartes revisees'], 0) || 0;
          const tdQuests = document.createElement('td'); tdQuests.textContent = getLeaderboardField(d, ['QuÃªtes_quotidiennes', 'Quêtes_quotidiennes', 'Quetes_quotidiennes'], 0) || 0;
        ...
        function dedupeLeaderboardEntries(rows, scoreField){
          ...
          for(const raw of rows || []){
            ...
            if(!current){
              byPseudo.set(key, { ...(raw || {}) });
              continue;
            }

            const currentScore = Number(current?.[scoreField] || 0);
            const nextScore = Number(raw?.[scoreField] || 0);
            const currentCards = Number(getLeaderboardField(current, ['Cartes rÃ©visÃ©es', 'Cartes révisées', 'Cartes revisees'], 0) || 0);
            const nextCards = Number(getLeaderboardField(raw, ['Cartes rÃ©visÃ©es', 'Cartes révisées', 'Cartes revisees'], 0) || 0);
            const currentLevel = Number(getLeaderboardField(current, ['Niveau', 'Niveau PrÃ©pa', 'Niveau Prépa', 'Niveau Prepa'], 0) || 0);
            const nextLevel = Number(getLeaderboardField(raw, ['Niveau', 'Niveau PrÃ©pa', 'Niveau Prépa', 'Niveau Prepa'], 0) || 0);
            const preferRaw = nextScore > currentScore || (nextScore === currentScore && (nextCards > currentCards || (nextCards === currentCards && nextLevel > currentLevel)));
            const best = preferRaw ? (raw || {}) : current;
            ...
            merged.Niveau = Math.max(Number(best.Niveau || 0), Number(other.Niveau || 0));

            const mergedTitles = [];
            const seenTitles = new Set();
            for(const src of [best.Titres, other.Titres]){
              const arr = Array.isArray(src) ? src : [];
              for(const item of arr){
                const name = String(item?.nom || item?.name || '').trim();
                const tier = Number(item?.tier || 0);
                const k = `${name}::${tier}`;
                if(!name || seenTitles.has(k)) continue;
                seenTitles.add(k);
                mergedTitles.push(item);
              }
            }
            if(mergedTitles.length) merged.Titres = mergedTitles;

            const mergedBadges = Array.from(new Set([
              ...(Array.isArray(best.Badges) ? best.Badges : []),
              ...(Array.isArray(other.Badges) ? other.Badges : [])
            ].map(v => String(v || '').trim()).filter(Boolean)));
            if(mergedBadges.length) merged.Badges = mergedBadges;

            if(!merged.Selected_Title){
              merged.Selected_Title = other.Selected_Title || '';
            }

            byPseudo.set(key, merged);
          }
        ...
        function attachGlobal(){
          ...
          unsubGlobal = qg.onSnapshot(snapshot=>{
            try{
              const allData = [];
              snapshot.forEach(doc=>{ const d = doc.data() || {}; if(isValidLeaderboardEntry(d)) allData.push(d); });
              const uniqueData = dedupeLeaderboardEntries(allData, 'Score_MPSI');
              uniqueData.sort((a,b) => Number(b.Score_MPSI || 0) - Number(a.Score_MPSI || 0));
              tbody.innerHTML = '';
              let r = 1;
              uniqueData.forEach(d => {
                const row = renderRow(d, r);
    ...
    async function removeWelcome(){
      try{
        ...
        // hide online indicator when leaving welcome page
        const indicator = document.getElementById('onlineIndicator'); if(indicator) indicator.remove();
  ...
  function createOnlineIndicator(){
    ...
    const welcomePage = document.getElementById('welcomeDecks');
    if(!welcomePage) return;
    
    const indicator = document.createElement('div');
    indicator.id = 'onlineIndicator';
    indicator.style.cssText = 'position:absolute;top:50px;left:10px;padding:4px 8px;border-radius:6px;font-size:0.7em;font-weight:600;display:flex;align-items:center;gap:4px;z-index:100;transition:all 0.3s;box-shadow:0 2px 6px rgba(0,0,0,0.12);';
    ...
    text.id = 'statusText';
    text.style.display = 'none'; // Hide text, keep only dot
